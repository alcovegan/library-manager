...
- [ ] [P1] Расширенная схема книги: `title`, `authors[]`, `series`, `seriesIndex`, `year`, `publisher`, `isbn`, `format`, `language`, `genres[]`, `tags[]`, `rating`, `notes`, `location`.
...
- [ ] [P2] Рейтинг и заметки: звездочки/оценка и текстовые заметки.

## Поиск и организация
- [ ] [P0] Быстрый поиск: fuzzy‑поиск (Fuse.js) по названию/авторам/ISBN в рендерере.
...
- [ ] [P1] По ISBN: автозаполнение с Open Library/Google Books (обложка, авторы, год, издатель) с кэшированием.
...
## UX и темизация
- [ ] [P1] Детальная карточка/панель: модалка/слайд‑овер с метаданными, файлами, журналом.
- [ ] [P1] Режимы просмотра: сетка/список, плотность, сортировки (название, автор, дата добавления, серия).
- [ ] [P1] Клавиатурные шорткаты: N — новая книга, / — поиск, Del — удалить, Cmd/Ctrl+F — фокус поиска.
...
## Данные и надёжность
- [ ] [P0] Переход на SQLite: `better-sqlite3` в `main`; таблицы `books`, `authors`, `book_authors`, `files`, `loans`; индексы (`title`, `isbn`).
- [ ] [P0] Транзакции/очередь записи: при JSON — атомарная запись через временный файл и `rename`; при SQLite — транзакции.
- [ ] [P0] Миграции: версия схемы `schema_version` и последовательные миграции при старте.
- [ ] [P0] Идентификаторы: UUID v4 для связей и внешних файлов.

## Безопасность и права
- [ ] [P0] Доступ к ФС только в `main`: все операции файлов — через IPC; в рендерере без Node.
...
- [ ] [P0] SQLite (main): `src/main/db.js` — инициализация, миграции, CRUD; IPC: `books:list|create|update|delete`, `authors:list`, `files:add|remove`.
- [ ] [P1] Импорт ISBN: `src/main/providers/openlibrary.js`, IPC `meta:byIsbn`, кэш в таблице `isbn_cache`.
- [ ] [P1] Импорт CSV: `src/main/importers/csv.js` — парсер, маппинг колонок, dry‑run + отчёт.
- [ ] [P0] Поиск (renderer): `src/renderer/search.js` — Fuse.js, фильтры в памяти; для больших коллекций — IPC‑фильтры по SQLite.
