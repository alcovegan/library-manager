...
function render() {
  ...
  for (const b of list) {
    ...
    const hasCollections = bookCollections.length > 0;
    collectionsBtn.className = hasCollections ? 'icon-btn bookmarked' : 'icon-btn';
    collectionsBtn.title = hasCollections ? `В коллекциях: ${bookCollections.join(', ')}` : 'Добавить в коллекцию';
